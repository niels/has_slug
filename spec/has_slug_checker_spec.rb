...
# specifically to test rescue_from_slug_mismatch functionality
describe SluggedController, 'SlugMismatchError exception handling', :type => :controller do
...
# In CustomSluggedController we defined our own rescue_from_slug_mismatch block
describe CustomSluggedController, 'customized SlugMismatchError exception handling', :type => :controller do
  
  it "should redirect to a custom target when faced with an invalid or incomplete id parameter" do
    # This is necessary in at least rspec <= 1.1.11 to "unignore" rescue_handlers
    controller.use_rails_error_handling!
    
    get :show, :id => @invalid_ids.first
    response.should redirect_to("http://test.host/somewhere-custom")
  end
